function eeg_App()
    ...
    function detectarEventos(~, ~)
        ...
        for v = 1:n_ventanas
            ...
            es_patologica(v) = energia_delta > 60; 
        end
        ...
        for v = 1:n_ventanas
            if es_patologica(v)
                ...
                if duracion_actual >= 10
                    crisis_detectadas = crisis_detectadas + 1;
                    tabla_ev{end+1, 1} = round(tiempo_inicio_crisis, 2);
                    tabla_ev{end,   2} = round(tiempo_inicio_crisis + duracion_actual, 2);
                    tabla_ev{end,   3} = duracion_actual;
                    tabla_ev{end,   4} = 'Crisis EEG';
                end
